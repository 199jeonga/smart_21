...
- after를 사용할 경우 `:nth-child(1) || first-chlid`를 사용해도 된다.
...
- float:right; 를 사용하면 후에 a에게 bp:b를 주면 틀어진다.
  - 해결방안 : position
- width:0, 혹은 1px (0을 준 이유는 border를 적용시키기 때문)
...
- `li가 아닌 a에 적용할 때 생기는 문제` border까지 클릭하는 범위 안에 들어간다.
...
포커스를 줄 수 없는 요소에게 강제로 주는 ||  있는 요소를 강제로 삭제하는 || 우선순위를 주는 요소
...
**transform은 값을 모른 채로도 가운데로 오게 지정할 수 있다. (IE6~8 X)**
...
`' '` 문자로 인식, js용어가 아니라면 모두 작은따옴표 안에 입력
