...
`border : O` = 부모를 데리고 마진 탑 진행 (부모요소가 첫번째라면 그 부모도 데리고 내려온다.)

`border : 0, none` = 부모 내에서 첫번째 요소만 마진 탑 진행



#### margin 중복 적용

`일반 블럭 요소` 첫번째 요소의 margin-bottom과, 두번째 요소의 margin-top이 중첩됐을 때 <u>큰 px가</u> 적용된다.
